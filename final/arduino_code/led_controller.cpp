...
// --- Sunlight Color Presets ---

CRGB SUNLIGHT_NEUTRAL = CRGB(255, 244, 229);  // Soft white (morning/evening)
CRGB SUNLIGHT_MIDDAY  = CRGB(255, 255, 255);  // Pure white (midday)
CRGB SUNLIGHT_WARM    = CRGB(255, 220, 180);  // Warm white (sunset tone)
...
/**
 * Converts a color name string to its corresponding CRGB color.  Defaults to white if not found.
 */
...
/**
 * Parses a command string from Serial and triggers the appropriate effect.
 */
...
/**
 * Lights up a growing circular shape from the center (row, col).
 * Uses polar coordinates and delays between each radius expansion.
 */
...
/**
 * Lights up an expanding ellipse centered at (row, col).
 * Uses parametric math to grow in x and y radius independently.
 */
...
/**
 * Fades all LEDs to a new color smoothly, using a blending ratio.
 */
...
/**
 * Lights each column exclusively one at a time, turning the rest off.
 */
